/*
 ...
 File Name  : P08R7.c
 Description: Source code for P08R7 module, implementing IR Time-of-Flight (ToF) sensor (ST VL53L1CX).
...
uint16_t P08R7_distance = 0;

/* Module Parameters */
ModuleParam_t ModuleParam[NUM_MODULE_PARAMS] = {
    { .ParamPtr = &P08R7_distance, .ParamFormat = FMT_UINT16, .ParamName = "distance" }
};
...
Module_Status Module_MessagingTask(uint16_t code, uint8_t port, uint8_t src, uint8_t dst, uint8_t shift)
{
  Module_Status result = P08R7_OK;
  uint32_t Numofsamples;
  uint32_t timeout;

  switch (code)
  {
	case CODE_P08R7_SAMPLE_PORT:
		SampleToPort(cMessage[port - 1][shift], cMessage[port - 1][1 + shift]);
		break;

	default:
		result = P08R7_ERR_UNKNOWNMESSAGE;
		break;
  }
...
Module_Status GetModuleParameter(uint8_t paramIndex, float *value) {
    ...
    switch (paramIndex) {
        ...
        case 1:
        {
            ...
            if (status == P08R7_OK) *value = (float)temp;
            break;
        }

        /* Invalid parameter index */
        default:
            status = P08R7_ERR_WRONGPARAMS;
            break;
    }
...
/***************************************************************************/
 Module_Status StreamToCLI(uint32_t Numofsamples, uint32_t timeout,
	...
	while ((numTimes-- > 0) || (timeout >= MAX_TIMEOUT_MS)) {
		...
		if (PollingSleepCLISafe(period,numTimes) != P08R7_OK)
			break;
	}
 ...
 */
Module_Status SampleToTerminal(uint8_t dstPort, SampleToString dataFunction) {
	Module_Status status = P08R7_OK; /* Initialize operation status as success */
...
Module_Status SampleToPort(uint8_t dstModule, uint8_t dstPort) {
    ...
    if (dstPort == 0 && dstModule == myID) {
        return P08R7_ERR_WRONGPARAMS; /* Return error for invalid parameters */
    }
    ...
    if (dstModule == myID) {
        ...
        MessageParams[1] = (status == P08R7_OK) ? BOS_OK : BOS_ERROR;   /* Operation status */
        MessageParams[2] = 1;                                           /* Number of elements (distance) */
        MessageParams[3] = (uint8_t)(CODE_P08R7_SAMPLE_PORT );          /* Command code LSB */
        MessageParams[4] = (uint8_t)(CODE_P08R7_SAMPLE_PORT >> 8);      /* Command code MSB */
...
Module_Status StreamToPort(uint8_t dstModule, uint8_t dstPort, uint32_t numOfSamples, uint32_t streamTimeout) {
	...
	if ((NULL == xTimerStream) || (0 == streamTimeout) || (0 == numOfSamples))
		return P08R7_ERROR; /* Assuming P08R7_ERROR is defined in Module_Status */
